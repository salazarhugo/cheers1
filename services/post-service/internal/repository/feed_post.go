...
func (p *postRepository) FeedPost(
	...
	result := db.
		Table("posts").
		Select(
			"posts.*, ARRAY(?) AS medias, drinks.DrinkId as drink_id, drinks.name as drink_name, drinks.icon as drink_icon, users.username, users.name, users.verified, users.picture, (?) as like_count, (?) as has_viewer_liked",
			mediaQuery,
			likeCountQuery,
			hasViewerLikedQuery,
		).
